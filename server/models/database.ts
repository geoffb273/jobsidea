...
const addUser = async function ({
  ...
  //, phone, birthday, profilePic = undefined) {
  const p = [];
...
const getUsersByLocation = function (
  zipCodes: string[],
  limit?: number,
  offset?: number,
) {
...
const putMessage = function (chatId: string, author: string, msg: string) {
  ...
  return postItem(db, 'Messages', msgObj).then(_ => {
    return msgObj;
  });
};
...
const getNotifications = async function (
  username: string,
  callback: (snapshot: Document) => void,
  limit?: number,
) {
  const snapshot = await getList(
    db,
    'Notifications',
    { username: username },
    { created: -1 },
...
const putNotification = function (
  username: string,
  sender: string,
  msg: string,
  type: string,
...
const uploadProfilePic = function (
  username: string,
  id: string,
  ...
  const promises = [];
  promises.push(
    updateItem(db, 'Users', { username: username }, { $set: { pic: id } }),
  );
...
const uploadResume = function (
  ...
  const promises = [];
  promises.push(
    updateItem(db, 'Users', { username: username }, { $set: { resume: id } }),
  );
  promises.push(
    uploadImage('resumes', id, file, { contentType: 'application/pdf' }),
  );
  return Promise.all(promises);
};
